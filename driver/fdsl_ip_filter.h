...
#define FDSL_TF_BUCKET_SIZE 500
...
struct fdsl_ip_filter {
	struct fdsl_ip_filter_ele *elements[FDSL_TF_BUCKET_SIZE];
	int ip_ver;
};

#define FDSL_TF_ELE_SIZE sizeof(struct fdsl_ip_filter_ele)
#define FDSL_TF_SIZE sizeof(struct fdsl_ip_filter)
...
struct fdsl_ip_filter *fdsl_tf_init(int ip_ver)
{
	struct fdsl_ip_filter *f;
	if (FDSL_IP_VER4 != ip_ver && FDSL_IP_VER6 != ip_ver) return NULL;

	f = (struct fdsl_ip_filter *)fdsl_malloc(FDSL_TF_SIZE);
...
int fdsl_tf_find(struct fdsl_ip_filter *f, const char *s)
{
...
int fdsl_tf_add(struct fdsl_ip_filter *f, const char *s)
{
	...
	if (NULL == f->elements[bucket_p]) {
		f->elements[bucket_p] = (struct fdsl_ip_filter_ele *)fdsl_malloc(FDSL_TF_ELE_SIZE);
	...
	if (tmp->have) {
		tmp->next = (struct fdsl_ip_filter_ele *)fdsl_malloc(FDSL_TF_ELE_SIZE);
...
void fdsl_tf_release(struct fdsl_ip_filter *f)
{
